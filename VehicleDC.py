...
class Manager(object):
    ...
    def predict(self, img):
        ...
        # get result
        pred = self.get_predict(output)  # self.get_predict_ce, return pred to host side(cpu)
...
class Car_DC():
    def __init__(self,
        ...
        self.Net.net_info['height'] = self.inp_dim  # set input dimension of image
        self.Net.to(device)
        self.Net.eval()  # evaluation mode
        print('=> car detection model initiated.')

        # initiate multilabel classifier
        self.manager = Manager(num_cls=19,
                               model_path=local_model_path)
